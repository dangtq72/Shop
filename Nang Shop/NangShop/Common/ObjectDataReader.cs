...
{
    //MinhND
    //Lớp này có chức năng đẩy các đối tượng (List hoặc Array) vào đối tượng IDataReader để thực hiện BulkSQLCopy.
    public class ObjectDataReader<TData> : IDataReader
    {
        ...
        public ObjectDataReader(IEnumerable<TData> data)
        {
            ...
            var propertyAccessors = typeof(TData)
                ...
                .Where(p => p.CanRead)
                .Select(
                         (p, i) => new { Index = i,Property = p, Accessor = CreatePropertyAccessor(p) }
                       )
                .ToArray();
